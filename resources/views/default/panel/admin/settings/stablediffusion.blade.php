...
@section('settings')
    ...
    >
        ...
        <div class="row">
            ...
            @php
                $stableEntities = \App\Domains\Entity\EntityStats::image()->filterByEngine(\App\Domains\Engine\Enums\EngineEnum::STABLE_DIFFUSION)->list();
            @endphp
            <div class="col-md-12">
                <div class="mb-3">
                    ...
                    >
                        @foreach($stableEntities as $entity)
                            <option
                                value="{{ $entity->enum()->slug() }}"
                                {{ $settings_two->stablediffusion_default_model === $entity->enum()->slug() ? 'selected' : null }}
                            >
                                {{ $entity->enum()->label() }}
                            </option>
                        @endforeach
